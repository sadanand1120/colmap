...
void AddCeresProgress(const std::string& label,
  ...
  if (options->caspar) {
    options->caspar->progress_callback =
        [progress, state, label, max_num_solves, solve_index = 0](
            const int current, const int total) mutable {
          if (solve_index == 0) {
            std::lock_guard<std::mutex> lock(state->mutex);
            solve_index = ++state->num_solves_started;
          }
          progress(
              StringPrintf("%s solve %d/%d",
                           label.c_str(),
                           std::min(solve_index, std::max(max_num_solves, 1)),
                           std::max(max_num_solves, 1)),
              std::min(current, std::max(total, 1)),
              std::max(total, 1));
        };
  }
}
...
GlobalMapperOptions InitializeOptions(const GlobalMapperOptions& options) {
  ...
  opts.rotation_averaging = options.RotationAveraging();
  opts.global_positioning = options.GlobalPositioning();
  opts.bundle_adjustment = options.BundleAdjustment();
  opts.retriangulation = options.Retriangulation();
  return opts;
}
...
void GlobalMapper::EstablishTracks(const GlobalMapperOptions& options,
  ...
  for (const auto& [track_length, point3D_id] : track_lengths) {
    ...
    if (reconstruction_->NumPoints3D() >= max_num_tracks) {
      ReportProgress(progress_callback,
                     "Selecting tracks",
                     track_lengths.size(),
                     track_lengths.size());
